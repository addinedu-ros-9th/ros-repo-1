...
class RobotCommander(BasicNavigator): # Node 대신 BasicNavigator를 직접 상속받아
    def __init__(self):
        ...
        self.waypoints = {
            'A1': (-5.3862, 6.3501, 0.0),
            'A2': (-3.3422, 6.4722, 0.0),
            'A3': (0.1064, 6.2238, 0.0),
            'B1': (-5.3761, 4.6925, 0.0),
            'B3': (0.0757, 5.0622, 0.0),
            'C1': (-5.4063, 3.5777, 0.0),
            'C3': (0.1, 3.2450, 0.0),
            'C4': (1.9483, 3.2892, 0.0),
            'C5': (3.2429, 3.234, 0.0),
            'C6': (4.6446, 4.6981, 0.0),
            'C7': (5.9597, 3.1693, 0.0),
            'C8': (7.502, 3.1582, 0.0),
            'C9': (9.2177, 2.7412, 0.0),
            'D3': (0.15, 1.7411, 0.0),
            'D5': (3.2418, 1.6709, 0.0),
            'D7': (6.0458, 1.6253, 0.0),
            'D9': (9.309, 1.7148, 0.0),
            'E3': (0.088, 0.0743, 0.0),
            'E4': (1.9111, 0.0166, 0.0),
            'E5': (3.3497, 0.0123, 0.0),
            'E6': (4.53, 0.0679, 0.0),
            'E7': (5.9853, 0.5879, 0.0),
            'E8': (7.758, 0.5479, 0.0),
            'E9': (9.2161, 0.5687, 0.0),
        }
        ...
        self.srv = self.create_service(NavigateSrv, 'navigate', self.navigate_callback)

        # '/goal_pose' 토픽을 구독하는 subscriber 생성
    ...
    def goal_pose_callback(self, msg): # 명령 msg 메세지 받으면 실행
        """'/goal_pose' 토픽을 구독하여 네비게이션 명령을 처리하는 콜백 함수"""
        # 이미 다른 네비게이션 작업이 진행 중인 경우, 새 명령을 무시해.
        if self.is_navigating:
            self.get_logger().warn('이미 다른 네비게이션 작업을 수행중입니다. 토픽 명령을 무시합니다.')
            return

        self.get_logger().info(f"토픽으로 명령 받음: X={msg.x}, Y={msg.y}, Yaw={msg.yaw}")
