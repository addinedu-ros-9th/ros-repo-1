 ...
 * @date      2025/07/31
 * @version   1.3 (Stream Start Logic Added)
 */
...
int CameraPublisher::onCameraOpen(AS_CAM_PTR pCamera)
{
    ...
    // =================================================================
    // ### 해결책: 여기서 모든 필요한 스트림을 명시적으로 시작합니다. ###
    // =================================================================
    unsigned int stream_flags = DEPTH_IMG_FLG | RGB_IMG_FLG; // 뎁스와 RGB 스트림을 모두 요청
    ret = AS_SDK_StartStream(pCamera, stream_flags);
    if (ret != 0) {
        RCLCPP_ERROR(this->get_logger(), "스트림 시작 실패! Error code: %d", ret);
        return ret;
    }
    RCLCPP_INFO(this->get_logger(), "RGB와 Depth 스트림을 성공적으로 시작했습니다.");
    // =================================================================
