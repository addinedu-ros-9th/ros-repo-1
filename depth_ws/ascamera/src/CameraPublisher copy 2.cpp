...
// CameraPublisher 생성자: UDP 소켓 초기화
CameraPublisher::CameraPublisher() : Node("camera_publisher_udp")
{
    initLaunchParams(); // 런치 파라미터 초기화
    ...
    if (rgb_sock_ < 0) {
        RCLCPP_ERROR(this->get_logger(), "RGB 소켓 생성 실패");
    }
    memset(&rgb_dest_addr_, 0, sizeof(rgb_dest_addr_));
    rgb_dest_addr_.sin_family = AF_INET;
    rgb_dest_addr_.sin_port = htons(9090); // RGB 전송 포트
    if (inet_pton(AF_INET, dest_ip, &rgb_dest_addr_.sin_addr) <= 0) {
        RCLCPP_ERROR(this->get_logger(), "잘못된 RGB 목적지 주소");
    }

    // 2. Depth 이미지용 소켓 설정
    depth_sock_ = socket(AF_INET, SOCK_DGRAM, 0);
    if (depth_sock_ < 0) {
        RCLCPP_ERROR(this->get_logger(), "Depth 소켓 생성 실패");
    }
    memset(&depth_dest_addr_, 0, sizeof(depth_dest_addr_));
    depth_dest_addr_.sin_family = AF_INET;
    depth_dest_addr_.sin_port = htons(9091); // Depth 전송 포트
    if (inet_pton(AF_INET, dest_ip, &depth_dest_addr_.sin_addr) <= 0) {
        RCLCPP_ERROR(this->get_logger(), "잘못된 Depth 목적지 주소");
    }
}
...
// onCameraDetached: 카메라 연결 해제 시 호출
...
// onCameraOpen: 카메라 열림 콜백 (해상도 설정 등)
...
int CameraPublisher::setResolution(AS_CAM_PTR pCamera, LAUNCH_CONFI_PARAM_S para)
{
    ...
    // IR, Peak 등 다른 스트림 설정도 필요 시 추가 가능
    return ret;
}

// logCameraPathInfo: 카메라 경로 정보 로깅
...
// initLaunchParams: 런치 파일 파라미터 초기화
...
// printLaunchParams: 읽어온 런치 파라미터 출력
...
// 아래는 원래 코드에 있었으나 UDP 전송 방식에서는 직접적으로 필요 없는 콜백 함수들입니다.
// SDK가 이 함수들을 요구할 수 있으므로, 비워두거나 최소한의 로깅만 남겨둡니다.
int CameraPublisher::onCameraClose(AS_CAM_PTR pCamera) { 
    (void)pCamera; 
    RCLCPP_INFO(this->get_logger(), "카메라 닫힘"); 
    return 0; 
}
int CameraPublisher::onCameraStart(AS_CAM_PTR pCamera) { 
    (void)pCamera; 
    RCLCPP_INFO(this->get_logger(), "스트림 시작"); 
    return 0; 
}
int CameraPublisher::onCameraStop(AS_CAM_PTR pCamera) { 
    (void)pCamera; 
    RCLCPP_INFO(this->get_logger(), "스트림 중지"); 
    return 0; 
}
