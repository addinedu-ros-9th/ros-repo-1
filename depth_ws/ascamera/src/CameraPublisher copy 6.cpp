 ...
 * @brief     angstrong camera publisher with UDP streaming improvements.
 *
 ...
 * @version   1.0
 * 
 * UDP IMPROVEMENTS IMPLEMENTED:
 * - Image compression (JPEG for RGB, PNG for Depth) to reduce packet size
 * - Enhanced error handling with errno reporting
 * - Socket health monitoring and automatic reconnection
 * - Retry mechanism for failed transmissions
 * - Larger socket buffers for better performance
...
// UDP MODIFIED: Add headers for UDP socket programming and image compression
...
// UDP MODIFIED: Define target IP and ports
#define UDP_TARGET_IP "127.0.0.1"
#define RGB_UDP_PORT 12345
#define DEPTH_UDP_PORT 12346
...
// UDP MODIFIED: These member variables should be added to the CameraPublisher class definition in CameraPublisher.h
// int rgb_socket_fd;
// int depth_socket_fd;
// struct sockaddr_in rgb_servaddr;
// struct sockaddr_in depth_servaddr;
...
CameraPublisher::CameraPublisher() : Node("camera_publisher")
{
    ...
    // UDP MODIFIED: Initialize UDP sockets and server address structures with enhanced error handling
    LOG(INFO) << "Initializing UDP sockets for RGB (port " << RGB_UDP_PORT << ") and Depth (port " << DEPTH_UDP_PORT << ")" << std::endl;
    
    // Create RGB UDP socket
    if ((rgb_socket_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        LOG(ERROR) << "RGB UDP socket creation failed: " << strerror(errno) << std::endl;
        rgb_socket_fd = -1;
    } else {
        LOG(INFO) << "RGB UDP socket created successfully (fd: " << rgb_socket_fd << ")" << std::endl;
        
        // Set socket options for larger buffer size
        int buffer_size = 1024 * 1024; // 1MB buffer
        if (setsockopt(rgb_socket_fd, SOL_SOCKET, SO_SNDBUF, &buffer_size, sizeof(buffer_size)) < 0) {
            LOG(WARN) << "Failed to set RGB socket send buffer size: " << strerror(errno) << std::endl;
        }
        
        memset(&rgb_servaddr, 0, sizeof(rgb_servaddr));
        rgb_servaddr.sin_family = AF_INET;
        rgb_servaddr.sin_port = htons(RGB_UDP_PORT);
        if (inet_aton(UDP_TARGET_IP, &rgb_servaddr.sin_addr) == 0) {
             LOG(ERROR) << "RGB UDP inet_aton failed for IP: " << UDP_TARGET_IP << std::endl;
             close(rgb_socket_fd);
             rgb_socket_fd = -1;
        } else {
            LOG(INFO) << "RGB UDP target configured: " << UDP_TARGET_IP << ":" << RGB_UDP_PORT << std::endl;
        }
    }

    // Create Depth UDP socket
    if ((depth_socket_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        LOG(ERROR) << "Depth UDP socket creation failed: " << strerror(errno) << std::endl;
        depth_socket_fd = -1;
    } else {
        LOG(INFO) << "Depth UDP socket created successfully (fd: " << depth_socket_fd << ")" << std::endl;
        
        // Set socket options for larger buffer size
        int buffer_size = 1024 * 1024; // 1MB buffer
        if (setsockopt(depth_socket_fd, SOL_SOCKET, SO_SNDBUF, &buffer_size, sizeof(buffer_size)) < 0) {
            LOG(WARN) << "Failed to set Depth socket send buffer size: " << strerror(errno) << std::endl;
        }
        
        memset(&depth_servaddr, 0, sizeof(depth_servaddr));
        depth_servaddr.sin_family = AF_INET;
        depth_servaddr.sin_port = htons(DEPTH_UDP_PORT);
        if (inet_aton(UDP_TARGET_IP, &depth_servaddr.sin_addr) == 0) {
            LOG(ERROR) << "Depth UDP inet_aton failed for IP: " << UDP_TARGET_IP << std::endl;
            close(depth_socket_fd);
            depth_socket_fd = -1;
        } else {
            LOG(INFO) << "Depth UDP target configured: " << UDP_TARGET_IP << ":" << DEPTH_UDP_PORT << std::endl;
        }
    }
}
...
{
    // UDP MODIFIED: Close UDP sockets
    if (rgb_socket_fd >= 0) {
        close(rgb_socket_fd);
    }
    if (depth_socket_fd >= 0) {
        close(depth_socket_fd);
    }
        ...
        AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
        if (it->pCamera == pCamera) {
            ...
            // UDP MODIFIED: Send Depth Image via UDP with compression and enhanced error handling
            if (it->index == 0 && depth_socket_fd >= 0) { // Only send for device 0 as requested
                try {
                    // Create OpenCV Mat from the depth image data (16-bit depth data)
                    cv::Mat depth_mat(pstData->depthImg.height, pstData->depthImg.width, CV_16UC1, (void*)pstData->depthImg.data);
                    
                    // Compress to PNG with compression level (PNG supports 16-bit depth)
                    std::vector<uchar> compressed_data;
                    std::vector<int> compression_params = {cv::IMWRITE_PNG_COMPRESSION, 3}; // Compression level 0-9
                    
                    bool compress_success = cv::imencode(".png", depth_mat, compressed_data, compression_params);
                    
                    if (compress_success && !compressed_data.empty()) {
                        size_t compressed_size = compressed_data.size();
                        LOG(INFO) << "Depth image compressed: " << stDepthImage.data.size() << " -> " << compressed_size 
                                  << " bytes (ratio: " << (float)compressed_size/stDepthImage.data.size() << ")" << std::endl;
                        
                        // Check if compressed size is within UDP limits
                        const size_t UDP_MAX_SIZE = 65000; // Leave some room for headers
                        if (compressed_size < UDP_MAX_SIZE) {
                            // Try sending with socket health check and reconnection
                            ...
                            while (!send_success && retry_count < MAX_RETRIES) {
                                ...
                                if (!checkSocketHealth(depth_socket_fd, "Depth")) {
                                    LOG(WARN) << "Depth socket unhealthy, attempting reconnection..." << std::endl;
                                    if (!recreateDepthSocket()) {
                                        LOG(ERROR) << "Failed to recreate Depth socket, skipping frame" << std::endl;
                                        break;
                                    }
                                }
                                
                                ssize_t sent_bytes = sendto(depth_socket_fd, compressed_data.data(), compressed_size, 0,
                                                           (const struct sockaddr *)&depth_servaddr, sizeof(depth_servaddr));
                                
                                if (sent_bytes < 0) {
                                    LOG(ERROR) << "Failed to send depth data via UDP (attempt " << (retry_count + 1) 
                                              << "): " << strerror(errno) << " (attempted " << compressed_size << " bytes)" << std::endl;
                                    retry_count++;
                                    if (retry_count < MAX_RETRIES) {
                                        std::this_thread::sleep_for(std::chrono::milliseconds(10)); // Brief delay before retry
                                    }
                                } else if ((size_t)sent_bytes != compressed_size) {
                                    LOG(WARN) << "Partial depth UDP send: " << sent_bytes << "/" << compressed_size << " bytes" << std::endl;
                                    send_success = true; // Partial send is still considered success
                                } else {
                                    LOG(INFO) << "Depth UDP send successful: " << sent_bytes << " bytes" << std::endl;
                                    send_success = true;
                                }
                            }
                        } else {
                            LOG(WARN) << "Compressed depth image still too large for UDP: " << compressed_size 
                                      << " bytes (max: " << UDP_MAX_SIZE << ")" << std::endl;
                        }
                    } else {
                        LOG(ERROR) << "Failed to compress depth image for UDP transmission" << std::endl;
                    }
                } catch (const cv::Exception& e) {
                    LOG(ERROR) << "OpenCV exception in depth UDP processing: " << e.what() << std::endl;
                } catch (const std::exception& e) {
                    LOG(ERROR) << "Exception in depth UDP processing: " << e.what() << std::endl;
                }
            }
            ...
            if (rclcpp::ok()) {
                ...
                // UDP MODIFIED: The original publish call is commented out.
                /*
                                       ...
                                       AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    // UDP MODIFIED: Send RGB Image via UDP with compression and enhanced error handling
    for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
        if (it->pCamera == pCamera) {
            if (it->index == 0 && rgb_socket_fd >= 0) { // Only send for device 0 as requested
                try {
                    // Create OpenCV Mat from the RGB image data
                    cv::Mat rgb_mat(pstData->rgbImg.height, pstData->rgbImg.width, CV_8UC3, (void*)pstData->rgbImg.data);
                    
                    // Compress to JPEG with quality setting
                    std::vector<uchar> compressed_data;
                    std::vector<int> compression_params = {cv::IMWRITE_JPEG_QUALITY, 80}; // 80% quality
                    
                    bool compress_success = cv::imencode(".jpg", rgb_mat, compressed_data, compression_params);
                    
                    if (compress_success && !compressed_data.empty()) {
                        size_t compressed_size = compressed_data.size();
                        LOG(INFO) << "RGB image compressed: " << rgbImage.data.size() << " -> " << compressed_size 
                                  << " bytes (ratio: " << (float)compressed_size/rgbImage.data.size() << ")" << std::endl;
                        ...
                        if (compressed_size < UDP_MAX_SIZE) {
                            ...
                            while (!send_success && retry_count < MAX_RETRIES) {
                                // Check socket health before sending
                                if (!checkSocketHealth(rgb_socket_fd, "RGB")) {
                                    LOG(WARN) << "RGB socket unhealthy, attempting reconnection..." << std::endl;
                                    if (!recreateRgbSocket()) {
                                        LOG(ERROR) << "Failed to recreate RGB socket, skipping frame" << std::endl;
                                        break;
                                    }
                                }
                                
                                ssize_t sent_bytes = sendto(rgb_socket_fd, compressed_data.data(), compressed_size, 0,
                                                       (const struct sockaddr *)&rgb_servaddr, sizeof(rgb_servaddr));
                                
                                if (sent_bytes < 0) {
                                    LOG(ERROR) << "Failed to send RGB data via UDP (attempt " << (retry_count + 1) 
                                              << "): " << strerror(errno) << " (attempted " << compressed_size << " bytes)" << std::endl;
                                    retry_count++;
                                    if (retry_count < MAX_RETRIES) {
                                        std::this_thread::sleep_for(std::chrono::milliseconds(10)); // Brief delay before retry
                                    ...
                                    LOG(WARN) << "Partial RGB UDP send: " << sent_bytes << "/" << compressed_size << " bytes" << std::endl;
                                    send_success = true; // Partial send is still considered success
                                } else {
                                    LOG(INFO) << "RGB UDP send successful: " << sent_bytes << " bytes" << std::endl;
                                    send_success = true;
                                }
                            }
                        } else {
                            LOG(WARN) << "Compressed RGB image still too large for UDP: " << compressed_size 
                                      << " bytes (max: " << UDP_MAX_SIZE << ")" << std::endl;
                        }
                    } else {
                        LOG(ERROR) << "Failed to compress RGB image for UDP transmission" << std::endl;
                    }
                } catch (const cv::Exception& e) {
                    LOG(ERROR) << "OpenCV exception in RGB UDP processing: " << e.what() << std::endl;
                } catch (const std::exception& e) {
                    LOG(ERROR) << "Exception in RGB UDP processing: " << e.what() << std::endl;
                }
            }
            break; // Found the matching publisher info, no need to continue loop
        }
    }
...
int CameraPublisher::streamController()
{
    ...
    while (rclcpp::ok()) {
        /*
         * UDP MODIFIED: 이 로직은 ROS2 토픽 구독자를 확인하여 스트림을 제어합니다.
         * UDP 통신을 위해 스트림이 항상 켜져 있도록 전체를 주석 처리합니다.
         */
        /*
        server->getLock().lock();
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
...
// UDP MODIFIED: Socket reconnection utility functions
bool CameraPublisher::checkSocketHealth(int socket_fd, const std::string& socket_name)
{
    if (socket_fd < 0) {
        LOG(WARN) << socket_name << " socket is invalid (fd: " << socket_fd << ")" << std::endl;
        return false;
    }
    ...
    if (retval != 0 || error != 0) {
        LOG(WARN) << socket_name << " socket health check failed: " << strerror(error) << std::endl;
        return false;
    }
...
bool CameraPublisher::recreateRgbSocket()
{
    LOG(INFO) << "Recreating RGB UDP socket..." << std::endl;
    ...
    // Create new socket
    if ((rgb_socket_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        LOG(ERROR) << "Failed to recreate RGB UDP socket: " << strerror(errno) << std::endl;
        return false;
    }
    ...
    rgb_servaddr.sin_port = htons(RGB_UDP_PORT);
    if (inet_aton(UDP_TARGET_IP, &rgb_servaddr.sin_addr) == 0) {
        LOG(ERROR) << "Failed to configure RGB UDP address during recreation" << std::endl;
        close(rgb_socket_fd);
        rgb_socket_fd = -1;
        return false;
    }
    
    LOG(INFO) << "RGB UDP socket recreated successfully (fd: " << rgb_socket_fd << ")" << std::endl;
    return true;
}

bool CameraPublisher::recreateDepthSocket()
{
    LOG(INFO) << "Recreating Depth UDP socket..." << std::endl;
    ...
    // Create new socket
    if ((depth_socket_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        LOG(ERROR) << "Failed to recreate Depth UDP socket: " << strerror(errno) << std::endl;
        return false;
    }
    ...
    depth_servaddr.sin_port = htons(DEPTH_UDP_PORT);
    if (inet_aton(UDP_TARGET_IP, &depth_servaddr.sin_addr) == 0) {
        LOG(ERROR) << "Failed to configure Depth UDP address during recreation" << std::endl;
        close(depth_socket_fd);
        depth_socket_fd = -1;
        return false;
    }
    
    LOG(INFO) << "Depth UDP socket recreated successfully (fd: " << depth_socket_fd << ")" << std::endl;
    return true;
}
