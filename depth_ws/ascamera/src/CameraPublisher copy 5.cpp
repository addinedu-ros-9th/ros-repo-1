...
// UDP MODIFIED: Add headers for UDP socket programming
...
CameraPublisher::CameraPublisher() : Node("camera_publisher")
{
    ...
    // UDP MODIFIED: Initialize UDP sockets and server address structures
    // This assumes rgb_socket_fd, depth_socket_fd, rgb_servaddr, and depth_servaddr are member variables.
    
    // Create RGB UDP socket
    if ((rgb_socket_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        LOG(ERROR) << "RGB UDP socket creation failed" << std::endl;
    } else {
        ...
        if (inet_aton(UDP_TARGET_IP, &rgb_servaddr.sin_addr) == 0) {
             LOG(ERROR) << "RGB UDP inet_aton failed" << std::endl;
             close(rgb_socket_fd);
             rgb_socket_fd = -1;
        }
    }
    ...
    if ((depth_socket_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        LOG(ERROR) << "Depth UDP socket creation failed" << std::endl;
    } else {
        ...
        if (inet_aton(UDP_TARGET_IP, &depth_servaddr.sin_addr) == 0) {
            LOG(ERROR) << "Depth UDP inet_aton failed" << std::endl;
        ...
        AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
        if (it->pCamera == pCamera) {
            ...
            // UDP MODIFIED: Send Depth Image via UDP instead of publishing to a ROS topic
            // NOTE: This sends the entire image in one go. If the image size exceeds the UDP packet
            // limit (~64KB), it might fail or be fragmented. A more robust implementation would
            // involve custom packetization with sequence numbers for reassembly on the client side.
            if (it->index == 0 && depth_socket_fd >= 0) { // Only send for device 0 as requested
                ssize_t sent_bytes = sendto(depth_socket_fd, stDepthImage.data.data(), stDepthImage.data.size(), 0,
                                           (const struct sockaddr *)&depth_servaddr, sizeof(depth_servaddr));
                if (sent_bytes < 0) {
                    // LOG(ERROR) << "Failed to send depth data via UDP"; // Optional: log error
                }
            }
                                       ...
                                       AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    // UDP MODIFIED: Send RGB Image via UDP instead of publishing to a ROS topic
    // NOTE: This sends the entire image in one go. If the image size exceeds the UDP packet
    // limit (~64KB), it might fail or be fragmented. A more robust implementation would
    // involve custom packetization.
    for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
        if (it->pCamera == pCamera) {
            if (it->index == 0 && rgb_socket_fd >= 0) { // Only send for device 0 as requested
                ssize_t sent_bytes = sendto(rgb_socket_fd, rgbImage.data.data(), rgbImage.data.size(), 0,
                                       (const struct sockaddr *)&rgb_servaddr, sizeof(rgb_servaddr));
                if (sent_bytes < 0) {
                    // LOG(ERROR) << "Failed to send RGB data via UDP"; // Optional: log error
                }
            }
